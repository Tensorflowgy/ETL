...
class jwNews(scrapy.Spider):
    name = "jwPaperSpider"
    start_url = "http://epaper.jwb.com.cn/jwb/html/%s-%s/%s/node_1.htm"
    header = spiderUtil.header_util()

    def start_requests(self):
        list = []
        begin_date = datetime.datetime.strptime("2015-09-01", "%Y-%m-%d")
    ...
    def parse(self, response):
        if response.status == 200:
            ...
            try:
                title = response.xpath("//founder-title/p[@class='BSHARE_TEXT']/text()").extract()[0].strip()
            except:
