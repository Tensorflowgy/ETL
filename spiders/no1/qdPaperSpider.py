...
class qdNews(scrapy.Spider):
    name = "qdPaperSpider"
    start_url = "http://www.dailyqd.com/epaper/html/%s-%s/%s/node_2.htm"
    header = spiderUtil.header_util()

    def start_requests(self):
        list=[]
        begin_date = datetime.datetime.strptime("2014-02-27", "%Y-%m-%d")
    ...
    def parse_item_home(self, response):
        paper_list = response.xpath("//ul[@id='dzb_bm_ww']/li/a/@href").extract()
    ...
    def parse_item_list(self, response):
        news_list = response.xpath("//ul/li/span/a/@href").extract()
    ...
    def parse(self, response):
        if response.status == 200:
            ...
            try:
                title = response.xpath("//table/tbody/tr/td")[0].xpath('string(.)').extract()[0].strip()
            except:
