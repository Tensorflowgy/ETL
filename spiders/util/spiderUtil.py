...
# es_index
es_index = "paper_news_data"
# 日志保存位置及日志文件名
logname="paper_news_data"
# logpath = "F:\ideaworkspace"
...
def save2es_news_data(es_index, title, source, data_id, author, public_time, content, domain, praise_num, url, ip,
    ...
    try:
        ...
        logger_info = "|" + es_index + "|" + crawl_time + "|" + str(html_size) + "|" + ip + "|" + str(
            es_size) + "|" + url + "|1|SUCCESS" + "|&|" + title + "|&|" + source + "|&|" + data_id + "|&|" + author + "|&|" + public_time + "|&|" + content + "|&|" + domain + "|&|" + praise_num + "|&|" + negative + "|&|" + polarity + "|&|" + positive + "|&|" + keywords + "|&|" + keyword_md5 + "|&|" + summary + "|&|" + source_type
...
def get_yesterday_date():
    tim = datetime.datetime.now()
    yesterday_date = (tim + datetime.timedelta(day=-1)).strftime("%Y-%m-%d")
