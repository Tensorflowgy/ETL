...
class dfNews(scrapy.Spider):
    name = "dfPaperSpider"
    start_url = "http://epaper.dongfeng.net/html/%s-%s/%s/node_2.htm"
    header = spiderUtil.header_util()

    def start_requests(self):
        list = []
        begin_date = datetime.datetime.strptime("2017-01-03", "%Y-%m-%d")
    ...
    def parse_item_list(self, response):
        news_list = response.xpath("//td[@class='btsy']/a/@href").extract()
    ...
    def parse(self, response):
        if response.status == 200:
            ...
            source = "http://epaper.dongfeng.net/"
