...
class klNews(scrapy.Spider):
    name = "klPaperSpider"
    start_url = "http://epaper.kelamayi.com.cn/klmyrb/html/%s-%s/%s/node_201.htm"
    header = spiderUtil.header_util()

    def start_requests(self):
        list = []
        begin_date = datetime.datetime.strptime("2012-01-16", "%Y-%m-%d")
    ...
    def parse_item_list(self, response):
        news_list = response.xpath("//area/@href").extract()
    ...
    def parse(self, response):
        if response.status == 200:
            ...
            source = "http://epaper.kelamayi.com.cn/"
            ...
            try:
                title = response.xpath("//td[@class='font01']/text()").extract()[0].strip()
            except:
