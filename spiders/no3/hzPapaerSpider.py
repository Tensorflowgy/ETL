...
"""
@author: TongYao
@file:   hzPapaerSpider.py
@time:  2019-05-21 9:28 
@function: 杭州日报新闻爬虫
"""
import datetime
import random
...
class hzNews(scrapy.Spider):
    ...
    def parse_item_home(self, response):
        paper_list = response.xpath("//li/a[@target='_parent']/@href").extract()
    ...
    def parse_item_list(self, response):
        news_list = response.xpath("//ul[@class='page-list']/li/a/@href").extract()
    ...
    def parse(self, response):
        if response.status == 200:
            ...
            try:
                title_arr1 = response.xpath("//div[@class='head']/h1//text()").extract()
                title_arr2 = response.xpath("//div[@class='head']/h2//text()").extract()
                title_arr3 = response.xpath("//div[@class='head']/h3//text()").extract()
                title = "".join(title_arr1).strip() + "".join(title_arr2).strip() + "".join(title_arr3).strip()
            except:
